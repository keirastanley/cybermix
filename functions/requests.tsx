...
/** Function to add a new comment to a track
 * @param playlist See playlistType in "@/data/types.tsx"
...
/** Function to remove a comment from a track
 * @param playlist See playlistType in "@/data/types.tsx"
 * @param track_id The id of the track you want to remove a comment from
 * @param comment_id The id of the comment you want to remove
 * @return The updated playlist
 */
export async function removeComment(playlist: playlistDataType, track_id: string, comment_id: string) {
  const response = await fetch(`https://cybermix-backend/api/playlists/${playlist._id}?action=remove-comment`,
    {
      method: 'PATCH',
      headers: {
        'Content-Type': 'application/json'
      },
      body: JSON.stringify({ track_id: track_id, comment_id: comment_id })
    })
  const data = await response.json()
  return data.payload[0];
}
